...
class LowestDB:
	...
	def insert_rows(self, batch_values):
		## batch_values = [row1, row2, ...]
		if len(batch_values[0])-1 != len(self.columns):
			return -0x02  ## --- Different column amount
		...
		for row in batch_values:
			try:
				c.execute(self.__cmd_insertIntoValues_(), row)
				conn.commit()
			except sqlite3.IntegrityError:
				self.log.print('**DB ERROR: BAD RECORD.')
				print(row)
				print(row)
				continue
				# return -0x03  ## --- Hash existed or something else
